...
$homeData = $controller->getById((int)$id);
    ...
    <main class="flex-1 min-w-0 h-screen overflow-y-auto px-4 md:px-8 space-y-4 pb-4">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <form action="process.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <input type="hidden" name="id" value="<?php echo htmlspecialchars($homeData['id']); ?>">

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Title</label>
                        <input type="text" name="title" required
                            value="<?php echo htmlspecialchars($homeData['title']); ?>"
                    ...
                    <div>
                        ...
                        <div class="mb-3 flex items-center gap-3 p-2 border border-slate-100 rounded-xl bg-slate-50">
                            ...
                            <div class="text-[10px] text-slate-500 truncate">
                                Current image: <?php echo basename($homeData['image']); ?>
                            </div>
                        </div>
                        <?php endif; ?>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required
                        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"><?php echo htmlspecialchars($homeData['description']); ?></textarea>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Text Content</label>
                    <textarea name="text" rows="8" required
                        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"><?php echo htmlspecialchars($homeData['text']); ?></textarea>
                </div>
