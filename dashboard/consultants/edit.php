...
$consultantData = $controller->getById((int)$id);
    ...
    <main class="flex-1 min-w-0 h-screen overflow-y-auto px-4 md:px-8 space-y-4 pb-4">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="consultants"
                ...
                <span class="hidden sm:inline">Back to Consultants</span>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <form action="process.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <input type="hidden" name="id" value="<?php echo htmlspecialchars($consultantData['id']); ?>">

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Title</label>
                    <input type="text" name="title" required
                        value="<?php echo htmlspecialchars($consultantData['title']); ?>"
                ...
                <div>
                    ...
                    <div class="mb-3 flex items-center gap-3 p-2 border border-slate-100 rounded-xl bg-slate-50">
                        ...
                        <div class="text-[10px] text-slate-500 truncate">
                            Current image: <?php echo basename($consultantData['image']); ?>
                        </div>
                    </div>
                    <?php endif; ?>
                ...
                <div>
                    ...
                    <textarea name="description" rows="6" required
                        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"><?php echo htmlspecialchars($consultantData['description']); ?></textarea>
                </div>
