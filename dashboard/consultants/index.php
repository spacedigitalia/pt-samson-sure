...
// Get all consultants data
$consultants = $controller->getAll();
?>
<section class="flex min-h-screen overflow-hidden">
    ...
    <div class="flex-1 min-w-0 h-screen overflow-y-auto px-4 md:px-8 space-y-4">
        ...
        <!-- Consultants Header -->
        <div
            class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-50 via-violet-50 to-fuchsia-50 border border-purple-100/50 shadow-lg">
            <!-- Decorative background elements -->
            <div
                class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-purple-200/20 to-fuchsia-200/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2">
            </div>
            <div
                class="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-tr from-violet-200/20 to-purple-200/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2">
            </div>
            ...
            <div class="relative px-6 md:px-8 py-6 md:py-8">
                <div class="flex flex-col md:flex-row items-start md:items-center md:justify-between gap-4 md:gap-6">
                    <div class="flex items-start gap-4 flex-1">
                        <!-- Icon Container -->
                        <div
                            class="flex-shrink-0 w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-violet-600 shadow-lg flex items-center justify-center transform transition-transform hover:scale-105 hidden md:flex">
                            <i class='bx bx-user-circle text-2xl md:text-3xl text-white'></i>
                        </div>

                        <!-- Title and Description -->
                        <div class="flex flex-col flex-1 min-w-0">
                            <div class="flex items-center gap-3 mb-2">
                                <h1 class="text-2xl md:text-4xl font-bold text-slate-900 tracking-tight">Consultants
                                    Management</h1>
                                <span
                                    class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-100 text-purple-700 text-xs font-semibold">
                                    <i class='bx bx-check-circle text-sm'></i>
                                    Active
                                </span>
                            </div>
                            <p class="text-sm md:text-base text-slate-600 leading-relaxed max-w-2xl">
                                Kelola konten consultants Buat, edit, dan hapus informasi
                                konsultan yang tersedia.
                            </p>

                            <!-- Stats (if consultants exist) -->
                            <?php if (!empty($consultants)): ?>
                            <div class="flex items-center gap-4 mt-3 pt-3 border-t border-purple-100/50">
                                <div class="flex items-center gap-2 text-xs text-slate-600">
                                    <i class='bx bx-list-ul text-purple-500'></i>
                                    <span class="font-medium"><?php echo count($consultants); ?> Consultants</span>
                                </div>
                                <div class="flex items-center gap-2 text-xs text-slate-600">
                                    <i class='bx bx-time text-violet-500'></i>
                                    <span>Last updated:
                                        <?php echo date('M d, Y', strtotime($consultants[0]['created_at'])); ?></span>
                                </div>
                            </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="flex-shrink-0">
                        <a href="/dashboard/consultants/create.php"
                            class="group flex items-center gap-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-violet-600 px-5 py-3 text-sm font-semibold text-white hover:from-purple-700 hover:to-violet-700 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                            <i class='bx bx-plus text-lg group-hover:rotate-90 transition-transform duration-300'></i>
                            <span class="hidden sm:inline">Add New Consultant</span>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-slate-600 max-w-md line-clamp-2">
                                    <?php echo htmlspecialchars(mb_substr($consultant['description'], 0, 100)); ?>
                                    <?php if (mb_strlen($consultant['description']) > 100): ?>...<?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-600">
                                    <?php echo htmlspecialchars($consultant['fullname'] ?? 'Unknown'); ?>
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-slate-500">
                                    <?php echo date('H:i', strtotime($consultant['created_at'])); ?>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="/dashboard/consultants/edit.php?id=<?php echo $consultant['id']; ?>"
                                    ...
                                    <form action="process.php" method="POST" class="inline"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus consultant ini?');">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-12 text-center">
            ...
            <p class="text-sm text-slate-500 mb-4">
                Belum ada konten consultants. Klik "Add New" untuk membuat konten pertama.
            </p>
            <a href="/dashboard/consultants/create.php"
